...
using std::string;
using std::chrono::duration_cast;  //函数duration_cast()提供了在不同的时间单位之间进行转换的功能。
using std::chrono::microseconds;   //微秒
using std::chrono::steady_clock;  //steady_clock是单调的时钟，相当于秒表；只会增长，适合用于记录程序耗时；

// signal handler
void sig_handler(int);  //标志处理器
#ifdef _DEBUG
void dump_history(int); //转储历史记录
#endif
...
// all in milliseconds
double QUOTA = 250.0; //时间配额为0.25秒
double MIN_QUOTA = 100.0; //最小时间配额0.1秒
double WINDOW_SIZE = 10000.0; //一个窗口的时间为10秒
int verbosity = 0;

#define EVENT_SIZE sizeof(struct inotify_event)
#define BUF_LEN (1024 * (EVENT_SIZE + 16))
auto PROGRESS_START = steady_clock::now();   //PROGRESS_START：获取系统时间戳，单位纳秒(nanosecond)
char limit_file_name[PATH_MAX] = "resource-config.txt";  //配置文件名称
char limit_file_dir[PATH_MAX] = ".";  //配置文件所在的文件夹
...
// milliseconds since scheduler process started 从进程开始经历了多长时间（ms表示）
...
void ClientInfo::update_return_time(double overuse) {
  ...
  for (auto it = history_list.rbegin(); it != history_list.rend(); it++) {
    if (it->name == this->name) {
      ...
      it->end = std::min(now, it->end + overuse);   //更新后的end时间为（之前的end+已经使用的时间）和现在的时间的最小值
      latest_actual_usage_ = it->end - it->start;  //最近实际使用的时间
      break;
    }
  }
  latest_overuse_ = overuse; //overuse为已经使用的时间
#ifdef _DEBUG
...
// self-adaptive quota algorithm 自适应配额调整算法
...
std::list<candidate_t> candidates;  //候选列表
...
void read_resource_config() {  //读取资源配置文件
  ...
  size_t gpu_memory_size;  //gpu显存申请大小
  double gpu_min_fraction, gpu_max_fraction; //gpu_request、gpu_limit
  int container_num; //容器数量
...
void monitor_file(const char *path, const char *filename) {
  ...
  // Initialize Inotify
  fd = inotify_init();  // Inotify是一个 Linux特性,它监控文件系统操作,比如读取、写入和创建。
...
candidate_t select_candidate() {
  while (true) {
    ...
    //如果等待队列candidates中第一个容器最近没有使用过GPU，则将令牌调度到该容器
    auto check_appearance = [=](const History &h) -> bool {
      return h.name == candidates.front().name;
    };
    if (std::find_if(history_list.begin(), history_list.end(), check_appearance) == history_list.end()) {
      candidate_t selected = candidates.front();
      candidates.pop_front();
      return selected;
    }

    // sort by time 对容器的时间戳进行由小到大排序
